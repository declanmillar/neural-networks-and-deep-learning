...
def regularized_dbl_conv():
    for lmbda in [0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]:
        for j in range(3):
            print("Conv + Conv + FC num %s, with regularization %s" %
                  (j, lmbda))
            net = Network(
                [
                    ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28),
...
def expanded_data(n=100):
    ...
    for j in range(3):
        print(
            "Training with expanded data, %s neurons in the FC layer, run num %s"
            % (n, j))
        net = Network(
            [
                ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28),
                ...
                ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12),
                              ...
                              activation_fn=ReLU),
                FullyConnectedLayer(
                    n_in=40 * 4 * 4, n_out=n, activation_fn=ReLU),
...
def ensemble(nets):
    ...
    for net in nets:
        ...
        net.test_predictions = list(
            np.concatenate([net.test_mb_predictions(i) for i in range(1000)]))
    all_test_predictions = list(zip(*[net.test_predictions for net in nets]))
    ...
    erroneous_predictions = [
        plurality(all_test_predictions[j]) for j in error_locations
    ]
...
def plot_errors(error_locations, erroneous_predictions=None):
    ...
    fig = plt.figure()
    error_images = [
        np.array(test_x[i]).reshape(28, -1) for i in error_locations
    ]
