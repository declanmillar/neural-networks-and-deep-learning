...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, validation_data, test_data, lmbda=0.0):
        ...
        self.test_mb_predictions = theano.function(
            [i], self.layers[-1].y_out, givens={self.x: test_x[i * self.mini_batch_size : (i + 1) * self.mini_batch_size]}
        )
...
class ConvPoolLayer(object):
    ...
    def __init__(self, filter_shape, image_shape, poolsize=(2, 2), activation_fn=sigmoid):
        ...
        self.w = theano.shared(
            np.asarray(np.random.normal(loc=0, scale=np.sqrt(1.0 / n_out), size=filter_shape), dtype=theano.config.floatX), borrow=True
        )
...
class FullyConnectedLayer(object):
    def __init__(self, n_in, n_out, activation_fn=sigmoid, p_dropout=0.0):
        ...
        # Initialize weights and biases
        self.w = theano.shared(
            np.asarray(np.random.normal(loc=0.0, scale=np.sqrt(1.0 / n_out), size=(n_in, n_out)), dtype=theano.config.floatX), name="w", borrow=True
        )
...
def dropout_layer(layer, p_dropout):
    srng = shared_randomstreams.RandomStreams(np.random.randint(999999))
